  ...
  if (urlObj.urlText.indexOf('migrationwiz.bittitan.com') == -1) { // KBs
    ...
    if (urlObj.urlText.split("/").length - 1 === 6) { // Item Level
      ...
      if (cntDoubleClick === 1) {
        ...
        browser.runtime.sendMessage({ action: 'open_new_tab', tabURL: action_url01 }, function (response) { });
        var action_url02 = matchProjOpt[1] + 'advancedOptions' + matchProjOpt[2] + '&returnRoute=project.index';
        browser.runtime.sendMessage({ action: 'open_new_tab', tabURL: action_url02 }, function (response) { });
        var action_url03 = matchLineOpt[1] + '/edit' + matchLineOpt[2];
        browser.runtime.sendMessage({ action: 'open_new_tab', tabURL: action_url03 }, function (response) { });
      }
    }
